...
pub fn solve(p: usize, s: usize) {
    ...
    if DFS {
        ...
        let mut stack = Vec::with_capacity(1 << p);
        let mut lengths = vec![1usize];

        stack.push((n_i, len_i));

        let mut new: Vec<(Int, usize)> = vec![];

        while stack.len() != 0 {
            let start_index = 0i64.max(stack.len() as i64 - BATCH_SIZE) as usize;

            use rayon::iter::*;
            
            new = stack.par_drain(start_index..).flat_map(|(n, len)| solve_bfs(n, len, p, s)).collect();
            stack.append(&mut new);

            lengths.push(stack.len());
            println!("{},", stack.len());
        }

        let lenlen = lengths.len();

        cgrustplot::plots::line_plot::line_plot(&lengths.into_iter().enumerate().map(|(i, len)| (i as f64, len as f64)).collect())
            .set_axes(true)
            .set_domain((0., lenlen as f64))
            .set_title(&format!("p{}_s{}", p, s))
            .as_image()
            .save(&format!("logs/p{}_s{}.png", p, s));
...
pub fn solve_bfs(n: Int, len: usize, p: usize, s: usize) -> Vec<(Int, usize)> {
    // len is the position (zero-indexed from right to left) of the first possible 1

    let no_sol = Vec::with_capacity(0);
    ...
    // Check Periodicity
    if n & mask != o & mask {return no_sol;}

    // Check Gaps (for concatonated solutions)
    if s == 0 && !gap_length_less_than(collected & mask, 2) {return no_sol;}
    ...
    if o == n {
        ...
        if min != n >> n.trailing_zeros() {return no_sol;}

        // No subperiodicity
        for row in &all[1..] {
            if min == *row >> row.trailing_zeros() {return no_sol;}
        }
    ...
    return vec![(n, new_len), (n | (one() << new_len), new_len)];
}